-- ============================================================================
-- ADICIONAR session_id EM paulo_orcamentos E paulo_inss (migração Z-API)
-- Rodar este arquivo INTEIRO no SQL Editor do Supabase.
--
-- Contexto: com a Z-API, o identificador único e estável do chat (cliente,
-- operador ou o próprio agente) é o chatLid, já tratado no n8n com
-- .replace('@lid', '') — o mesmo padrão usado em paulo_conversas_leads e
-- paulo_chat_histories. O telefone (p->>'telefone', vindo de Config.phone)
-- continua sendo gravado como está; session_id é uma coluna NOVA, adicional.
--
-- salvar_orcamento_v2() passa a aceitar e gravar p->>'session_id'.
-- O tool node no n8n (salvar_dados_obra2) precisa passar esse campo no
-- payload_json: "session_id": "{{ $('Config').item.json.chatLid.replace('@lid','') }}"
-- ============================================================================

-- 1. Novas colunas (idempotente)
ALTER TABLE paulo_orcamentos ADD COLUMN IF NOT EXISTS session_id TEXT;
ALTER TABLE paulo_inss       ADD COLUMN IF NOT EXISTS session_id TEXT;

CREATE INDEX IF NOT EXISTS idx_paulo_orcamentos_session_id ON paulo_orcamentos (session_id);

-- 2. Atualizar a função para gravar session_id em toda linha inserida
